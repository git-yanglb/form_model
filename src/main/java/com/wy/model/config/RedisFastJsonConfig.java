...
@Configuration
@EnableCaching
public class RedisFastJsonConfig extends CachingConfigurerSupport {
	...
	// 使用redis缓存时，RedisCacheManager生成RedisCache后生成缓存时默认使用
	// JdkSerializationRedisSerializer序列化（cache存储的时候）
	...
	public CacheManager cacheManager(RedisConnectionFactory connectionFactory) {
		...
		// RedisCacheConfiguration.defaultCacheConfig();//该语句相当于序列化方式1
		 ...
		 * fastjson默认情况下会开启autoType的检查，相当于一个白名单检查， 如果序列化信息中的类路径不在autoType中，
		 * 反解析就会报com.alibaba.fastjson.JSONException: autoType is not support的异常
		 * 可参考 https://blog.csdn.net/u012240455/article/details/80538540
		 */
		ParserConfig.getGlobalInstance().addAccept("com.boot.entity.");
